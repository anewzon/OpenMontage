...
// A channel opening for the scenic/relaxation grammar: a restrained,
// hierarchical title treatment over the episode's own footage. Deliberately quiet:
// opacity and scale only, staggered by hierarchy. No letter-by-letter reveal,
// no underline, no glow. Which footage the bed shows (moving water, a
// skyline, a fire, a road) is the channel's choice, stated in its BRAND.md
// channel-policy block - nothing here assumes a subject.
const { fontFamily } = loadFont("normal", {
  weights: ["400"],
  subsets: ["latin"],
});

export type ScenicOpeningProps = {
  /** Episode footage the type sits over - whatever bed the channel's BRAND.md asks for. */
  videoSrc: string;
...
export const calculateScenicOpeningMetadata: CalculateMetadataFunction<
  ScenicOpeningProps
> = async ({ props }) => {
  ...
  if (props.videoSrc && (width === undefined || height === undefined)) {
    try {
      ...
      throw new Error(
        `ScenicOpening could not read its bed ${props.videoSrc}: ${String(err)}`
      );
    }
  }
  if (!width || !height) {
    throw new Error(
      "ScenicOpening has no canvas: pass width/height/fps from the delivery " +
...
export const ScenicOpening: React.FC<ScenicOpeningProps> = ({
  videoSrc,
  brandSignature,
  welcomeMessage,
  episodeLine,
